...
const ContextMemo = memo(
	({ hasPlus, quotes, setWidgetReady, skipQuote, toggleQuoteFavourite }) => {
		...
		const currentQuote =
			quotes.find((quote) => moment(quote.forDate).isSame(moment(), "day")) ||
			(quotes.length &&
		...
		const toggleFavouriteQuote = () => {
			const isFavourite = heartIconRef.current.classList.toggle(ACTIVE);
			currentQuote?.id && toggleQuoteFavourite(currentQuote.id, isFavourite);
		};
		...
		return (
			<div className="app-container quote">
				<div className="app-dash">
					<p className="body">
						<span className="body-text">{`“${currentQuote.body}”`}</span>
						<span className="below u--touch-hide">
							...
							<span
								title={`${
									currentQuote?.isFavourite ? "Unfavourite" : "Favourite"
								}`}
								className={`control favorite ${
									currentQuote?.isFavourite ? ACTIVE : ""
								}`}
							...
							<span
								title="Share"
								className="control"
								onClick={shareHandler(currentQuote)}
								data-v-1040273e
							>
								<svg
